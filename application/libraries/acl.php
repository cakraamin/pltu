	...
	class ACL
	{
		...
		function getAllPerms($format='ids')
		{
			...
			while($row = mysql_fetch_assoc($data))
			{
				if ($format == 'full')
				{
					$resp[$row['permKey']] = array('ID' => $row['permID'], 'Name' => $row['permName'], 'Key' => $row['permKey']);
				} else {
					$resp[] = $row['permID'];
				}
			}
		...
		function getRolePerms($role)
		{
			...
			while($row = mysql_fetch_assoc($data))
			{
				...
				$perms[$pK] = array('perm' => $pK,'inheritted' => true,'value' => $hP,'Name' => $this->getPermNameFromID($row['permID']),'ID' => $row['permID'],'status' => $row['value'],'tanggal' => $row['addDate']);
			}
			return $perms;
		}
