...
class Mmaster extends CI_Model{
	...
	function getSkul($id,$num,$offset,$sort_by,$sort_order)//menu admin
	{
		if (empty($offset))
		{
			$offset=0;
		...
		$sort_columns = array('schools.id_school');
		$sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'schools.id_school';
		$sql = "SELECT * FROM schools WHERE jenjang_school='$id' ORDER BY $sort_by $sort_order LIMIT $offset,$num";
		$query = $this->db->query($sql);
		return $query->result();
	}

	function getNumSkul($id)
	{
