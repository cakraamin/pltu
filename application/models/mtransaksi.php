...

{
...

	{
		...
		$data = array(
			'tgltrans'	=> $this->input->post('tgltb1',TRUE),
	...
	function getCust($id)
	{
		$kueri = $this->db->query("SELECT * FROM tabcust WHERE namacust='$id'");
		if($kueri->num_rows() > 0)
		{
			$data = $kueri->row();
			return $data->kodecust;
		}
		else
		{
			$data = array(
	...
	function getTrsp($id)
	{
		$kueri = $this->db->query("SELECT * FROM tabtrsp WHERE namatrsp='$id'");
		if($kueri->num_rows() > 0)
		{
			$data = $kueri->row();
			return $data->kodetrsp;
		}
		else
		{
			$data = array(
