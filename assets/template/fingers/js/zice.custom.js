...
function validateAllSteps(){
    ...
    function validateSteps(step){
		...
		if(total != anak && anak != 0){			
			$('#pesan').html('<div class="alertMessage SE">Maaf Jumlah Tidak sesuai</div>');
			isStepValid = false;   
			return false;
		}		
		if(total != prodi && prodi != 0){			
			$('#pesan').html('<div class="alertMessage SE">Maaf Jumlah Tidak sesuai1</div>');
			isStepValid = false;   
			return false;
		}		
        $.post( "siswa/simpan_siswa/1", $( "#wizardsss"+step ).serialize(), function( data ) {
			if(data == 'ok'){
				isStepValid = true;   
			}else{
				isStepValid = false;   
